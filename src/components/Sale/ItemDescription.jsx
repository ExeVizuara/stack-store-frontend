        ...
        <div className="data sm:bg-[#262837] bg-[#466822d8] flex flex-col sm:p-2 lg:p-8 rounded-lg min-h-[350px] sm:min-h-[600px]">
            ...
            <div className="col-span-6 overflow-y-auto overflow-x-auto bg-gray-700 sm:p-4 min-h-[320px] sm:min-h-[445px] border border-gray-600">
                { products.map((product, index) => (
                    <SellProducts key={ index } { ...product } removeProduct={() => removeProduct(index, product, subTotal[product.id])} quantity={quantity[product.id]} addQuantity={() => addQuantity(product.id, product.price)} subtractQuantity={() => subtractQuantity(product.id, product.price)} subTotal={subTotal[product.id]} />
                ))}
            </div>
